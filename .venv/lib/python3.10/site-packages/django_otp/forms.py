...
class OTPAuthenticationFormMixin:
    ...
    def clean_otp(self, user):
        ...
        :param user: A user that has beenAuthenticated by the first factor
            (such as a password).
        :type user: :class:`~django.contrib.auth.models.User`
        :raises: :exc:`~django.core.exceptions.ValidationError` if the user is
            not fullyAuthenticated by an OTP token.
        """
        if user is None:
            return
    ...
    def _chosen_device(self, user):
        ...
        # have the list of choices until we Authenticatethe user. Without the
        # following, an attacker could Authenticateusing some other user's OTP
        # device.
...
class OTPAuthenticationForm(OTPAuthenticationFormMixin, AuthenticationForm):
    ...
    The intended behavior of the form is as follows:
        ...
        - Initially the ``username``, ``password``, and ``otp_token`` fields
          ...
          If we are able to Authenticatethe user based on username and password,
        ...
        - In either case, as long as the user isAuthenticated by their
          password, ``form.get_user()`` will return theAuthenticated
        ...
        - The user will then have to choose a specific device toAuthenticate
        ...
        - If the user presses any other submit button, we will Authenticatethe
...
class OTPTokenForm(OTPAuthenticationFormMixin, forms.Form):
    """
    A form that verifies anAuthenticated user. It looks very much like
    :class:`~django_otp.forms.OTPAuthenticationForm`, but without the username
    and password. The first argument must be anAuthenticated user; for an
    ...
    :param user: AnAuthenticated user.
    :type user: :class:`~django.contrib.auth.models.User`
